...
def _baseline_fingerprint(
    events: dict, capture_path: Path, window_s: float
) -> np.ndarray:
    ...
    for region in events["regions"]:
        if region["expected"] == "single":
            mask = ((ts - t0) >= region["start_s"]) & ((ts - t0) <= region["end_s"])
            if np.any(mask):
...
def run(
    ...
    for region in events["regions"]:
        ...
        windows_in_region = [
            w for w in timeline if eval_start <= w["window_start_s"] < re_
        ]
        ...
        if expected == "single":
            ok_frac = sum(
                1 for w in windows_in_region if w["similarity"] >= match_threshold
            ) / len(windows_in_region)
            label = f">= match_threshold ({match_threshold})"
        elif expected == "multi":
            ok_frac = sum(
                1 for w in windows_in_region if w["similarity"] < multi_threshold
            ) / len(windows_in_region)
    ...
    for r in region_summary:
        ...
        print(
            f"  {ok} [{r['start_s']:>4.0f}, {r['end_s']:>4.0f}] "
...
def main() -> None:
    ...
    p.add_argument(
        "--multi-threshold", type=float, default=DEFAULT_MULTI_SUBJECT_THRESHOLD
    )
    p.add_argument("--hysteresis-n", type=int, default=3)
    p.add_argument(
        "--json", type=Path, default=None, help="write timeline + region results here"
    )
    args = p.parse_args()
