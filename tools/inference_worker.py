...
def _load_model(model: str = "synthetic"):
    """Load the chosen model + feature index for HR confidence.

    Returns (predict_fn, hr_ratio_idx). predict_fn maps a (1, F) feature
    matrix to a float HR prediction.
    """
    ...
    try:
        ...
        pass
    log.info("loaded %s model from %s (%d features)",
             model, model_dir, len(feature_names))
    return hr, hr_ratio_idx


def run_once(window: _Window, fs_resample: float, window_s: float,
             hr_model, hr_ratio_idx: Optional[int]
             ) -> Optional[dict]:
    """Run the model on the current window contents.

    Returns the prediction payload, or None if there isn't enough data.
    """
    ...
    hr_conf = 0.0
    if hr_ratio_idx is not None and hr_ratio_idx < feats.shape[1]:
        hr_conf = float(np.clip(feats[0, hr_ratio_idx], 0.0, 1.0))
    return {
        "hr_bpm": round(hr, 2),
...
def loop(bus: MessageBus, patient_id: str, window_s: float, stride_s: float,
         fs_resample: float, hr_model, hr_ratio_idx: Optional[int],
         from_id: str = LATEST,
    ...
    out_topic = hr_predicted(patient_id)
    cursors = {in_topic: from_id}
    window = _Window(duration_s=window_s * 1.5)
    last_predict = 0.0
    iterations = 0
    log.info("worker for patient_id=%r: subscribing to %s, publishing to %s",
             patient_id, in_topic, out_topic)
...
def main() -> None:
    ...
    bus = bus_from_env()
    hr_model, hr_ratio_idx = _load_model(args.model)
    try:
        loop(
            bus=bus,
