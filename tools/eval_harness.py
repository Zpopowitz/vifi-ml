...
def _evaluate_session(session_dir: Path, model_dir: Path,
                      window_s: float = 10.0,
    ...
    if not capture_path.exists():
        return SessionResult(
            ...
            room_id=None, posture=None, distance_m=None,
            n_windows=0, n_suppressed=0,
            hr_mae_bpm=None, rr_mae_bpm=None,
            within_5_hr=None, within_2_rr=None,
    ...
    df_pred = [
        (w.window_start_s, w.hr_pred, w.suppressed)
...
def evaluate_directory(captures_root: Path, model_dir: Path,
                       window_s: float = 10.0,
                       stride_s: float = 5.0) -> list[SessionResult]:
    ...
    for subj_dir in sorted(captures_root.iterdir()):
        ...
        for sess_dir in sorted(subj_dir.iterdir()):
            ...
            try:
                r = _evaluate_session(sess_dir, model_dir,
...
def main() -> int:
    ...
    p.add_argument("--hr-mae-floor", type=float, default=None,
    ...
    if args.hr_mae_floor is not None:
        ...
        if bad:
            for r in bad:
                log.error("session %s/%s exceeded HR MAE floor: %.2f > %.2f",
                          r.subject_id, r.session_id,
                          r.hr_mae_bpm, args.hr_mae_floor)
            return 1
    return 0
