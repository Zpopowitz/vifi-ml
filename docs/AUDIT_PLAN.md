...
| 2 | **`config.py` orphan** — DSP constants (`TOP_K_SUBCARRIERS`, `EDGE_SUBCARRIER_GUARD`, `HR_BAND_HZ`, `RR_BAND_HZ`, `FFT_ZEROPAD_FACTOR`) imported by tests only | `config.py` | The whole point of `config.py` was to centralize the constants previously scattered across six DSP files. Tests reference it; the actual DSP code (`preprocess.py`) does not. **Recommendation: wire it in** — the centralization rationale still holds; the unfinished migration is the bug. |
...
| 4 | **mypy lenient modules with `\|\| true`** | `.github/workflows/ci.yml:51` | `security.py` + `audit.py` type-check but never fail CI. False confidence in the two files FDA cares most about. Either commit to strict types or remove the step. |
| 5 | **Ruff `format --check` deferred** | `.github/workflows/ci.yml:32` (comment) | Dead intent. The 70-file style sweep has been "tracked" without progress. Run it once and commit (one-time noise diff). Deferred-formatting in CI = QA gap for FDA. |
