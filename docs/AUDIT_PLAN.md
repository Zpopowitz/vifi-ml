...
| 7 | **`create_app()` was a 487-line monolith** — **PR-H phase 1 + 2 landed**. Phase 1 (a23d179) extracted the bundle classes; phase 2 (PR-H2) extracted the self-contained pieces — middleware install + SPA mount — into `api_internals/middleware.py` and `api_internals/spa.py`. api.py: 1265→1135 (130 lines extracted across both phases). Route handlers themselves are NOT split (they share closure variables with create_app for `real_bundle` / `synthetic_bundle` / `_resolve_calibration` / `_predict_capture`); attempting to extract those is the next refactor and would need a deps-injection pattern. For now the routes stay where they are — the noisy reviewer-friendly file-size reduction is what mattered. | `api.py`, `api_internals/{bundles,middleware,spa}.py` | |
