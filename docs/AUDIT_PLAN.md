...
| 7 | **`create_app()` is a 487-line monolith** — **PR-H phase 1 landed**: bundle classes extracted to `api_internals/bundles.py` (177 lines moved out, api.py 1265→1164). Phase 2 (route extraction) deferred as PR-H-routes — higher risk because routes share closure variables with create_app, and a botched refactor causes subtle prediction-path bugs. Will land when there's a quiet stretch with no other work in flight. | `api.py`, `api_internals/bundles.py` | |
