...
| 7 | **`create_app()` was a 487-line monolith** — **PR-H phases 1 + 2 + 3 landed**. Phase 1 (a23d179) extracted the bundle classes; Phase 2 (64f50bb) extracted middleware install + SPA mount; Phase 3 extracted the predict + identify + stub routes via a `register_*_routes(app, deps...)` factory pattern. api.py: 1265 → 1032 (-233 lines, -18%); the extracted code now lives in `api_internals/{bundles,middleware,spa,routes_predict,routes_stubs}.py` — six well-bounded files instead of one mixing-everything-together monolith. Phase 4 (`/health`, `/readyz`, `/api/v1/rooms`, WebSocket `/api/v1/stream`) is the next refactor; those have closure deps on `_warmup`, `_rooms_cache`, the audit writer, etc. — bigger surface area. | `api.py`, `api_internals/{bundles,middleware,spa,routes_predict,routes_stubs}.py` | |
