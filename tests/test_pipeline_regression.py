...
def _synth_envelope(
    ...
    return (0.5 * np.sin(2 * np.pi * freq_hz * t) + noise * rng.randn(len(t))).astype(
        np.float32
    )


def _synth_complex_csi(
    hr_freq_hz: float = 1.2,
    fs: float = 100.0,
    n_seconds: float = 10.0,
    n_sub: int = 192,
    seed: int = 42,
...
def test_apply_calibration_unity_at_self():
    feats = np.array(
        [0.3, 0.5, 1.2, 0.4, 100.0, 50.0, 200.0, 0.5, 8.0], dtype=np.float32
    )
...
def test_apply_calibration_works_on_2d_matrix():
    ...
    N = 5
    feats = np.tile(
        np.array([0.3, 0.5, 1.2, 0.4, 100.0, 50.0, 200.0, 0.5, 8.0], dtype=np.float32),
        (N, 1),
    )
    cal = np.array([0.9, 0.9, 0.9, 0.9, 50.0, 25.0, 100.0, 0.9, 7.0], dtype=np.float32)
    out = apply_calibration(feats, cal)
...
def test_full_pipeline_smoke_v1():
    ...
    cal_windows = np.stack(
        [extract_features(_synth_envelope(1.2, seed=i), fs=fs) for i in range(5)]
    )
